export class GenericDatasourceQueryCtrl {
  ...
  findMetrics(query: string) {
    return this.datasource.metricFindQuery(query, undefined, this.target.type, this.target.source);
  }

  findSources(query: string) {
    return this.datasource.sourceFindQuery(query, undefined, this.target.type);
  }
