...
export class GenericDatasource {
  ...
  metricFindQuery(query: string, options?: any, type?: string, source?: string): Promise<MetricFindValue[]> {
    const interpolated = {
      type,
      source,
  ...
  sourceFindQuery(query: string, options?: any, type?: string): Promise<MetricFindValue[]> {
    const interpolated = {
      type,
      target: this.templateSrv.replace(query, null, 'regex'),
    };
  ...
  buildQueryParameters(options: any) {
    options.targets = options.targets
      ...
      .filter((target: any) => {
        // remove placeholder targets
        return target.source !== 'select source';
      })
      .map((target: any) => {
        let data = null;
        ...
        let sourceValue = target.source;
        if (typeof sourceValue === 'string') {
          sourceValue = this.templateSrv.replace(target.source.toString(), options.scopedVars, 'regex');
        }
